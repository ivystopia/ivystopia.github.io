...
function Update-ChocolateyPackages {
    ...
    .DESCRIPTION
    Uses Chocolatey to upgrade itself and all installed packages individually, and keeps track of upgraded packages.
    ...
    # Function to retrieve outdated packages using --limit-output for easier parsing
    function Get-OutdatedPackages {
        param (
            [string]$Stage
        )

        Write-Host "Retrieving list of outdated packages ($Stage)..."
        $OutdatedOutput = choco outdated --limit-output

        if ($OutdatedOutput -match 'No packages found') {
            Write-Host "No outdated Chocolatey packages found ($Stage)."
            return @()
        }
        else {
            # Split the output into lines and trim each line
            $Packages = $OutdatedOutput -split "`n" | ForEach-Object { $_.Trim() }

            # Filter out any empty lines or lines that don't match package naming conventions
            # Assuming package names consist of alphanumerics, dots, dashes, and underscores
            $ValidPackages = $Packages | Where-Object { $_ -match '^[A-Za-z0-9._-]+$' }
    ...
    $OutdatedPackagesBefore = Get-OutdatedPackages -Stage "before upgrade"

    if ($OutdatedPackagesBefore.Count -eq 0) {
        Write-Host "No packages to upgrade."
    }
    else {
        Write-Host "Upgrading outdated packages individually..."
        foreach ($package in $OutdatedPackagesBefore) {
            ...
            try {
                ...
                Write-Host "Failed to upgrade: $package. Error: $_"
            }
        }
    }

    # Get list of outdated packages after upgrade
    $OutdatedPackagesAfter = Get-OutdatedPackages -Stage "after upgrade"
