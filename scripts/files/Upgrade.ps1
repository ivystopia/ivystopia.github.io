...
$ssdDrives = @("C", "E") # Maintained as per user's request
...
try {
    ...
    $WindowsUpdatesInstalled = Get-WindowsUpdate -AcceptAll -Install -IgnoreReboot -Verbose:$false
    ...
    Write-WarningMsg "Failed to install some Windows Updates."
}
Write-Separator

# ===========================================
#          Schedule Diagnostics and Disk Check
# ===========================================
Write-Info "Scheduling diagnostics and disk check..."
try {
    Start-Process -FilePath "mdsched.exe" -ArgumentList "/schedule" -NoNewWindow -Wait
    ...
    Write-WarningMsg "Failed to schedule diagnostics or initiate disk check."
}
Write-Separator
